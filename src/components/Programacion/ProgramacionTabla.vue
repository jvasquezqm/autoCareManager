...
<template>
  <div>
    <ProgramacionEditar ref="ProgramacionEditar"/>
    
    <q-table
      :rows="datosFiltrados"
      :columns="columns"
      row-key="ProgramacionId"
...
import ProgramacionEditar from 'src/components/Programacion/ProgramacionEdit.vue'


export default{

  name: "ProgramacionTabla",
  ...
  data(){
    return{
      ...
      busqueda: '',
      columns: [
        { name: 'id', align: 'center', label: 'ID', field: 'programacionId', sortable: true },
        { name: 'fechaProgramacion', align: 'center', label: 'Fecha', field: 'fechaProgramacion', sortable: true },
        { name: 'hora', align: 'center', label: 'Hora', field: 'hora', sortable: true },
        { name: 'vehiculoId', align: 'center', label: 'Vehículo', field: 'vehiculoId', sortable: true },
        { name: 'mecanicoId', align: 'center', label: 'Mecánico', field: 'mecanicoId', sortable: true },
  ...
  methods: {
    async getDatos(){
      try {
        const response = await axios.get('http://localhost:5243/api/Programacion/GetAll');
    ...
    celda(fila, columna){
      if (columna.field === 'programacion') {
        return fila.programacion.fechaProgramacion + ' ' + fila.programacion.hora;
      }
    ...
    columnasFiltrables() {
      return this.columns.filter((columna) => columna.field !== 'ProgramacionId');
    },

    editarFila(fila) {
      this.$refs.ProgramacionEditar.abrirModalEdicion(fila);
    ...
    eliminarFila(fila) {
      ...
      this.$q.dialog({
        ...
        try {
          const response = await axios.delete(`http://localhost:5243/api/Programacion/Delete${fila.programacionId}`);
