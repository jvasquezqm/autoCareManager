    ...
    <div>
      <q-dialog v-model="mostrarModal">
        <q-card class="qcard-modal-edit">
          <q-card-section>
            <q-input v-model="edicion.nombre" label="Nombre Mecánico" class="custom-label" />
            <q-input v-model="edicion.apellido" label="Apellido Mecánico" class="custom-label" />
            <q-input v-model="edicion.telefono" label="Teléfono" class="custom-label" />
            <q-select
            v-model="edicion.especialidad"
            label="Especilidad"
  ...
  export default {
    ...
    methods: {
      ...
      try {
        ...
        const response = await axios.put(`http://localhost:5243/api/Mecanico/Update${this.edicion.mecanicoId}`, this.edicion);
